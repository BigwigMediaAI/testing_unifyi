...
export const superAdminAPI = {
  ...
  updateUniversity: (id, data) =>
    api.put(`/superadmin/universities/${id}`, data),
...
export const universityAPI = {
  ...
  updateRegistrationConfig: (data) =>
    api.put("/university/config/registration", data),

  // Staff
  createStaff: (data) => api.post("/university/staff", data),
  listStaff: (role) => api.get("/university/staff", { params: { role } }),
  resetPassword: (userId, password) =>
    api.put(`/university/staff/${userId}/reset-password`, {
  ...
  updateDepartment: (id, data) =>
    api.put(`/university/departments/${id}`, data),

  // Courses
  createCourse: (data) => api.post("/university/courses", data),
  listCourses: (deptId) =>
    api.get("/university/courses", { params: { department_id: deptId } }),
...
export const leadAPI = {
  ...
  assign: (id, counsellorId) =>
    api.post(`/leads/${id}/assign`, { counsellor_id: counsellorId }),
  ...
  importCollegedunia: (leads) =>
    api.post("/leads/import/collegedunia", { source: "collegedunia", leads }),
};
...
export const applicationAPI = {
  ...
  updateBasicInfo: (id, data) =>
    api.put(`/applications/${id}/basic-info`, data),
...
export const queryAPI = {
  ...
  getMyQueries: (status) =>
    api.get("/queries/my-queries", { params: { status } }),
  getCounsellorQueries: (status) =>
    api.get("/queries/counsellor-queries", { params: { status } }),
...
export const walkinsAPI = {
  ...
  // Create walkin request
  create: (data) => {
    const token = localStorage.getItem("unify-token");

    return api.post("/walkins", data, {
      headers: {
  ...
  // Fetch student's own walkins
  getMy: () => {
    const token = localStorage.getItem("unify-token");

    return api.get("/walkins/my", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  // Fetch walkins assigned to counsellor
  getAssigned: () => {
    const token = localStorage.getItem("unify-token");

    return api.get("/walkins", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  // Approve / Modify / Reject walkin
  updateStatus: (walkinId, data) => {
    const token = localStorage.getItem("unify-token");

    return api.put(`/walkins/${walkinId}`, data, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
